...
impl Resolver {
    ...
    fn declare(&mut self, name: &Token) {
        ...
        match self.scopes.last_mut().unwrap().insert(name.raw.to_string(), false) {
            None => {},
            Some(_) => {
    ...
    fn define(&mut self, name: &Token) {
        ...
        self.scopes.last_mut().unwrap().insert(name.raw.to_string(), true);
    }

    fn resolve_local(&mut self, token: Token) -> Result<(), ResolverError> {
        for (i, scope) in self.scopes.iter().enumerate() {
            if scope.contains_key(&token.raw) {
                self.interpreter.borrow_mut().resolve(token, self.scopes.len() - 1 -i);
                return Ok(())
            }
        }
        Ok(())
    ...
    fn error(&self, token: Token, message: &str) -> ResolverError {
        println!("Resolver: {} caused by {} at line {} column {}", message, token.raw, token.line, token.column);
        lox::report_error();
    ...
    fn visit_stmt(&mut self, stmt: &stmt::Stmt) -> Result<(), ResolverError> {
        match stmt {
            ...
            stmt::Stmt::Break { token } => {
                if let ScopeType::None = self.current_scope {
                    Err(self.error(token.clone(), "Cannot break from top level code"))
                } else {
                    Ok(())
                }
            },
            stmt::Stmt::Return { return_value, token } => {
                if let FunctionType::None = self.current_function {
                    return Err(self.error(token.clone(), "Cannot return from top level code"));
                }
